...
<%@ page import="board.RecommendDAO" %>
<%@ page import="board.RecommendVO" %>
...
<form action="i_recommend.jsp">
<table>
         <tr>
            <td>추천수</td>
            <td><%= Recommand%>   </td>
            <td><input type="button" name="mi_recommand" value="좋아요!" onclick="document.location.href='i_recommend.jsp?mi_num=<%=article.getMi_num()%>&pageNum=<%=pageNum%>'"> 
            </td>
            <td><input type="button" name="mi_recommand" value="싫어요!" onclick="document.location.href='i_recommend2.jsp?mi_num=<%=article.getMi_num()%>&pageNum=<%=pageNum%>'"> 
            </td>
         </tr>
		...
		<form method="post" action="commentproc.jsp">
			...
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tr>
					...
					<td><input type="text" style="height:100px;" class="form-control" placeholder="상대방을 존중하는 댓글을 남깁시다." name = "content"></td>
...
List<RecommendVO> articleList = null;
...
if(count > 0) {
   ...
   if(startPage > pageBlock) {
      ...
      if(searchText == null) {
%>
      <a href="c_list.jsp?pageNum=<%=startPage-pageBlock%>">[이전]</a>
      <%
      }else {
      %>
      <a href="c_c_list.jsp?pageNum=<%=startPage-pageBlock%>&searchWhat=<%=searchWhat%>&searchText=<%=searchText%>"></a>
<%
      }
   }
   for(int i = startPage; i <= endPage; i++) {
      if(searchText == null) {
%>   
<a href="c_list.jsp?pageNum=<%=i%>">[<%=i%>]</a>
<%   }else { %>
   <a href="c_list.jsp?pageNum=<%=i%>&searchWhat=<%=searchWhat%>&searchText=<%=searchText%>"></a>
<% }
   }
   if(endPage < pageCount) {
      if(searchText == null) {
%>
<a href="c_list.jsp?pageNum=<%=startPage+pageBlock%>">[다음]</a>
<%}else { %>
   <a href="c_list.jsp?pageNum=<%=startPage+pageBlock%>&searchWhat=<%=searchWhat%>&searchText=<%=searchText%>"></a>
