...
if(count1 > 0) {
	...
	%>
	<a href="MainBoardList.jsp?pageNum=<%= startPage-pageBlock %>&m_searchWhat=<%=m_searchWhat%>&mserchbox=<%=mserchbox%>">[이전]</a>
	<% }
	for(int i = startPage; i <= endPage; i++){	
	%>		
	<a href="MainBoardList.jsp?pageNum=<%=i%>&m_searchWhat=<%=m_searchWhat%>&mserchbox=<%=mserchbox%>">[<%=i %>]</a>		
<% 	}
	if(endPage < pageCount){ %>	
	<a href="MainBoardList.jsp?pageNum=<%= startPage+pageBlock %>&m_searchWhat=<%=m_searchWhat%>&mserchbox=<%=mserchbox%>">[다음]</a>
<%} 
 }else{%>
<%} %>
<%

if(request.getParameter("m_searchBoard").equals("f_board")){ %>
<h3>자유게시판 검색결과</h3>
<%
...
if(request.getParameter("m_searchBoard").equals("r_board")){
%>
<%
